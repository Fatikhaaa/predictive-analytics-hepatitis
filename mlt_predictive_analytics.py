...
# Menentukan jumlah outlier untuk setiap kolom
# Kalkulasi lagi lower_bound and upper_bound using the current df and columns_to_winsorize
...
"""- Berdasarkan dari hasil evaluasi model diatas, terlihat model Random Forest dan K-Nearest Neighbor (KNN) menjadi model yang paling unggul dalam hal akurasi, presisi, recall, dan f1-score, diikuti oleh KNN, Decision Tree, dan Logistic Regression.
- Pemilihan ini didasarkan pada beberapa hal diantaranya, keunggulan model Random Forest dimana konsisten terhadap semua metrik, RF lebih tahan terhadap noise dan overfitting dibanding KNN dan dapat menganalisis pentingnya fitur (feature importance). Random Forest memiliki nilai Akurasi (0.9837), Precision (0.9840), Recall (0.9837) dan F1-Score (0.9832) tertinggi, mengindikasikan keseimbangan terbaik antara true positive dan true negative, serta false positive dan false negative. Model ini memberikan insight mengenai fitur yang paling relevan dan memberikan hasil yang optimal pada dataset yang diuji.
...
"""Terlihat ada sedikit peningkatan hasil evaluasi setelah dilakukan proses tuning pada model Random Forest, namun ada sedikit pengurangan pada Decision Tree. Model Logistic Regression dan KNN terlihat tidak ada perubahan sebelum dan setelah dilakukan tuning, ini menandakan model Random Forest yang menunjukkan hasil evaluasi terbaik.
